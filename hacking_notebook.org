...
#+RESULTS:
**** Unit test using CppUTest
***** Test driving =alloc=
We need first deduce test list from the specification and model:
- Allocate, freeing that, and then allocate again; two preceding allocation
  return same pointer.
- Allocate specific size and then allocate some size then the difference of them
  is same as specific size.
- Allocate some size and then store some value that fits in given size, it can
  be retrieve.
- Allocation leads overflow, it return null pointer.
- Freeing out of bound value ignored.


Then let's start to drive:
#+BEGIN_SRC C
TEST_GROUP(Alloc)
{
  char *allocated;
...
So far we used hard-coded implementation, following test force us to do real
job:
#+BEGIN_SRC C
...
TEST(Alloc, AllocLeadsOverFlow)
{
  ...
  POINTERS_EQUAL(NULL, overflowedAllocation);
}
#+END_SRC
...
char *alloc(int size)
{
    if (allocp + size >= allocBuffer + ALLOCSIZE)
        return NULL;
...
{
    if (allocated < allocBuffer ||
        allocated >= allocBuffer + ALLOCSIZE)
        return;
    allocp = allocated;
}
#+END_SRC

Now we refactor on green:
#+BEGIN_SRC C
...
#+END_SRC
***** Test driving IO functions
****** Test driving =getLine=
=Getline= read a line (less than limitation number of character) into given
character pointer and then return read character numbers.

To test this abstract behavior, here we use 0-1-N test strategy:
- 0 phase:
  - =Getline= returns 0 when read end of file character.
  - The string =getline= wrote when read end of file character is null string.
- 1 phase:
  - =Getline= returns 1 when read one character and end of file.
  - =Getline= returns 1 when read newline character.
  - The string =getline= wrote above situation is the string with one character
    read.
- N phase:
  - =Getline= do not read over the newline character or end of file character.
  - When read characters is less than limitation, =getline= return the number of
    characters read.
  - In above situation, the wrote string is same as input string until newline
    character or before the end of file character.
  - When number of readable characters is greater than limitation, it return
    limitation.
  - In above situation the string wrote is same as the string consisting of
    characters until limitation.
  - In above situation it does not read more characters than the number of
    limitation.


In addition to above test, as =getline= should read input from the user, we need
to provide test double that stubs the =getchar()= function of standard I/O
library. We can abstract out the =getchar= function using function pointer.

For now, we'll just assume that we have such test double and test drive the
=getline=; then if we got the concrete specification of such test double while
using that, we transit to implement test double, as usual.
...
TEST(Getline, OnlyEndOfChacter)
{
    ...
    LONGS_EQUAL(nChars, 0);
    STRCMP_EQUAL(lineRead, "");
}
#+END_SRC

From this we can deduce following interface: =CharInputSpy.h=
#+BEGIN_SRC C
#include "CharInput.h"
void CharInputSpy_Create(int);
void CharInputSpy_Destroy(void);
#+END_SRC

=CharInput.h=:
#+BEGIN_SRC C
extern int (*CharInput)(void);
#+END_SRC
...
That is, we applied dependency inversion to the direct dependency to =getchar=.

Now we can start to implement =CharInputSpy=:
#+BEGIN_SRC C
...
TEST(CharInputSpy, CreateWithNothing)
{
  ...
  int charGot = CharInput();

  LONGS_EQUAL(charGot, EOF);
}
#+END_SRC
...
Actually it passes as it is. We want to check whether =lineRead= is really
modified by =getLine=. That is, to make the test of current:
#+BEGIN_SRC C
TEST_GROUP(Getline)
{
    int nChars;
...
TEST(Getline, ReadNewlineCharacter)
{
    ...
    LONGS_EQUAL(1, nChars);
    STRCMP_EQUAL(expected, lineRead);
}
#+END_SRC

It, in turn, leads following test:
#+BEGIN_SRC C
...
Now we precede to the mainstream. Following code make the test pass
#+BEGIN_SRC C
int getLine(char *s, int limit)
{
    int c;
...
I realized that I should have checked the number of characters read:
#+BEGIN_SRC C
TEST(Getline, OnlyEndOfChacter)
{
    CharInputSpy_Create("");
...
Before apply this change to every existing test cases, let us first refactor
existing tests:
#+BEGIN_SRC C
...
It leads the implementation of spy:
#+BEGIN_SRC C
...
Before implement the =getLine= code, we need to complete test double to
encompass multiple input characters:
#+BEGIN_SRC C
...
As all the test so far passed, it's time to refactor. First, let's get rid of
=goto= statement:
#+BEGIN_SRC C
int getLine(char *s, int limit)
{
...
#+END_SRC
As all test passed as expected, we are safe to torch the old bridge:
#+BEGIN_SRC C
int getLine(char *s, int limit)
{
    int c;
...
These test just passes as it is.

Now we move on general case:
#+BEGIN_SRC C
...
Current implementation is alright for those tests. Finally we transit to last
test case, limitation check:
#+BEGIN_SRC C
...
However unlike what we expected from this, it leads
#+BEGIN_EXAMPLE
tests/IO/GetlineTest.cpp:118: error: Failure in TEST(Getline, CannotReadOverLimitation)
tests/IO/GetlineTest.cpp:32: error:
	LONGS_EQUAL(strlen(expected), nChars) failed
	expected < 0 (0x0)>
	but was  <21 (0x15)>

..............
Errors (1 failures, 18 tests, 18 ran, 35 checks, 0 ignored, 0 filtered out, 1 ms)
#+END_EXAMPLE

It somehow makes =expected= as ='\0'=; it is due to the peculiar behavior how
the =C= allocate stack memory. Let's think about what we misunderstood. The only
change is the limitation setting. Maybe our limitation interpretation is wrong
-- it might overread. Let's test that:
#+BEGIN_SRC C
int getLine(char *s, int limit)
{
    int c;
...
It seems we are on a right track. Actually the behind story is follows; we need
to spare null character space to indicate the end of string! So if the
limitation of space is given, says /x/, then we can not read more than that
limitation - 1 characters, that is /x - 1/.

So the final test changes as follows
#+BEGIN_SRC C
TEST(Getline, CannotReadOverLimitation)
{
    sprintf(expected, "This should be read. But it shouldn't");
...
Now all the test got passed, let's refactor the code. First get rid the
superfluous parts:
#+BEGIN_SRC C
int getLine(char *s, int limit)
{
...
I've realized afther those series of implementations, that I also have to check
=getLine= can be called multiple times:
#+BEGIN_SRC C
...
#+END_SRC
****** Test driving =readlines=
Let's first state how =readlines= should behave. =Readlines= is analogous
function to =getLine= in that it take pointer and limitation space and call
another submodule's function to get the contents from the user (standard input);
but, in addition, it also handle the dynamic allocation what read.

So our test list is analogous to =getLine=:
- 0 phase:
  - =Readlines= returns 0 when there is no line available.
  - In above situation, =readlines= do nothing to given pointer and return 0.
- 1 phase:
  - =Readlines= returns 1 when read one line and no more line and read line is
    able to be allocated.
  - In above situation, it allocate that line and store newly allocated pointer
    to given pointer array.
  - Otherwise, if there is no space to allocated read line, do nothing to the
    given pointer array and return error singal.
- N phase:
  - When number of lines read is less than limitation and read lines is
    allocatable, =readlines= return the number of lines read.
  - In above situation, all lines read allocated in heap and those line pointers
    is stored in given pointer array in order it read.
  - When number of readable lines is greater than limitation, it return
    error signal.
  - In above situation, the state of heap and given pointer array is
    undetermined; those are left as implementation details.
  - As 1 phase, whenever the read line is not allocatable, it return error flag.


Now we move on to the test:
#+BEGIN_SRC C
extern "C"
{
#include "Readlines.h"
...
{
    char *linePointers[MAXLINE] = { NULL };
    void setup()
    {
        UT_PTR_SET(CharInput, CharInputSpy);
    }
...
{
    sprintf(expected, "One line is available.");
    CharInputSpy_Create(expected);
    nLines = readlines(linePointers, MAXLINE);

    LONGS_EQUAL(1, nLines);
    STRCMP_EQUAL(expected, linePointers[0]);
    LONGS_EQUAL(strlen(expected), allocSpy_AllocatedSpace());
}
#+END_SRC

This test shows the components =readlines= depends on. As described in test
list, it depends on =alloc= and =getLine=, which in turn depends on =CharInput=.
So above test shows how we designed the stubbing dependencies of code under
test: Using =CharInputSpy= we indirectly input the characters, and using new
test double, =allocSpy=, we inspect the newly allocated space via =readlines=.
We could have chosen to inspect the number of =getLine= calls from =readlines=
by creating new test double; but it isn't in the specification it does not need
to be checked.

Here we need to implement =allocSpy=, which can inspect the whole allocated
space at specific time. To support such stubbing, our design of =readline=
should be flexible enough -- actually this is what so called /SOLID design
discipline/. So first we should apply /Dependency Inversion Principle/. By using
this scheme, we can change the implementation of allocation function as we need
-- e.g. to =malloc=.

Then here is the abstract module, =charAlloc.c=:
#+BEGIN_SRC C
#include "charAlloc.h"
#include "Alloc.h"
...
While, I'll detect the bug of =alloc=:
#+BEGIN_SRC C
char *alloc(int size)
{
    if (IsOutOfBounds(allocp + size - 1)) /* not allocp + size! */
        return NULL;
...
We should have counted =allocp= as available space.

Now we return to the test of =readlines=:
#+BEGIN_SRC C
TEST(Readlines, OneAvailableLineAllocatable)
{
    sprintf(characterInput, "One line is available.");
    CharInputSpy_Create(characterInput);
    nLines = readlines(linePointers, MAXLINE);

    LONGS_EQUAL(1, nLines);
    STRCMP_EQUAL(characterInput, linePointers[0]);
    LONGS_EQUAL(strlen(characterInput) + 1, // NULL character
...
int readlines(char **linePointers, int limitNumberOfLines)
{
    ...
    int lineLength = getLine(line, MAXLEN);
    if (lineLength == 0)
        return 0;

    currentAllocated = charAlloc(lineLength + 1); /* NULL character */
    strcpy(currentAllocated, line);
    linePointers[0] = currentAllocated;

    return 1;
}
#+END_SRC

And next we test 1 phase with no available space:
#+BEGIN_SRC C
...
Now let's move on to the multiple line read:
#+BEGIN_SRC C
TEST(Readlines, MultipleLinesAvailableAndAllocatable)
{
    expectedLines[0] = "One line is available.\n";
    expectedLines[1] = "No actually it was two lines as whole.";

    strcpy(characterInput, expectedLines[0]);
    strcat(characterInput, expectedLines[1]);

    CharInputSpy_Create(characterInput);
    nLines = readlines(linePointers, MAXLINE);

    LONGS_EQUAL(2, nLines);
    STRCMP_EQUAL(expectedLines[0], linePointers[0]);
    STRCMP_EQUAL(expectedLines[1], linePointers[1]);
...
consumeMore:
    lineLength = getLine(line, MAXLEN);
...
Since all the tests so far are green, it's time to refactor the test code:
#+BEGIN_SRC C
TEST_GROUP(Readlines)
{
    ...
    void setInputCharsAndReadlines()
    {
...
TEST(Readlines, MultipleLinesAvailableAndAllocatable)
{
    expectedLines[numberOfLines++] = "One line is available.\n";
    expectedLines[numberOfLines] = "No actually it was two lines as whole.";

    setInputCharsAndReadlines();

    checkExpectedAndActual();
}
#+END_SRC

Or, since our setting expectation does not match with other function's
abstraction level, we can wrap it as
#+BEGIN_SRC C
...
Now we move on to the next test case
#+BEGIN_SRC C
TEST(Readlines, MutipleLinesOverLimitation)
{
...
We check also the boundary cases:
#+BEGIN_SRC C
...
We checked all the test cases. It's time to refactor the code. Let us first
reduce the code to the coherent structure:
#+BEGIN_SRC C
int readlines(char **linePointers, int limitNumberOfLines)
{
    char line[MAXLEN];
...
Currently, I'm not sure how I should refactor above code any further; let me
delegate this task to myself of future.
****** Test driving =writelines=
Here we are going to use =FormatOutputSpy= to implement =writelines= test code.
...
{
    if (numberOfLines)
        FormatOutput(linePointers[0]);
}
#+END_SRC

At this point, I've realized that to work with =qsort=, we need to modify
=readlines= and =writelines= so that those can break long line into several
lines to prevent strange behavior that would occur with current implementations.
But it would need further changes to support those; for now let us move on with
those.
...
#+END_SRC
***** Test driving sorting procedure
Since, quick sort is sorting algorithm, there is no much space for TDD to part
in. So here we just verify the behavior of quick sort:
#+BEGIN_SRC C
...
Now let's put together. We take optional arguments by parsing commandline and
then sort the lines input from standard input and print those sorted lines to
the standard output.
...
{
  char expectedOutput[] = "One line does not need to be sorted.";
  CharInputSpy_Create(expectedOutput);
...
{
  char *fakeInput = (char *) malloc(MAXLINES * 10);
  int offset = 0;

  for (int i = 0; i < MAXLINES + 1; ++i)
  {
    int numberOfWritten = sprintf(fakeInput + offset, "%d lines\n", i);
    offset += numberOfWritten;
  }
  ...
  STRCMP_EQUAL("input too big to sort\n", FormatOutputSpy_GetOutput());
...
TEST(SortLines, CanCopeWithFullCase)
{
  ...
  LONGS_EQUAL(0, result);
  STRCMP_EQUAL(fakeInput, FormatOutputSpy_GetOutput());
...
int SortLines(void)
{
    ...
    quickSort((void **) linePointers, 0,
              numberOfLinesRead - 1,
              (int (*)(void *, void *))
              (SortLines_IsNumeric() ? numcmp : strcmp));
    writelines(linePointers, numberOfLinesRead);
    return 0;
}
#+END_SRC

Now we refactor the test codes since all the test are passed safely:
#+BEGIN_SRC C
TEST(SortLines, ParseNothing)
{
  char *fakeArgv[] = { "progname" };
...
#+END_SRC
**** Exercise 5-14
Now we move on to add new feature, to sort in reverse order. First we update the
parser:
#+BEGIN_SRC C
...
{
    int c;
    option.isNumeric = option.shouldReverse = 0;
    while (--argc > 0 && (*++argv)[0] == '-')
        while (c = *++argv[0])
            switch (c) {
                case 'n':
...
{
    return option.shouldReverse;
}
#+END_SRC

Now we move on to the behavior:
#+BEGIN_SRC C
...
Then make the compare procedure abstract to be dynamic:
#+BEGIN_SRC C
...
{
    if (postProcessor)
        return postProcessor(compareProc(s1, s2));
    return compareProc(s1, s2);
}
...
{
    if (SortLines_IsNumeric())
        compareProc = numcmp;
    else
        compareProc = (int (*)(char *, char *))strcmp;

    if (SortLines_ShouldReverse())
        postProcessor = invertOutput;
    else
        postProcessor = 0;
}
...
To cope with this behavior, we have experimented with abstract component:
#+BEGIN_SRC C
static void (*preProcessor)(char *to, char *from);
...
static int compare(char *s1, char *s2)
{
    ...
    preProcessor(localS1, s1);
    preProcessor(localS2, s2);

    return postProcessor(compareProc(localS1, localS2));
}
...
static void setCompareProcedure(void)
{
    if (SortLines_ShouldFold())
        preProcessor = toLowers;
    else
        preProcessor = (void (*)(char *, char *)) strcpy;
    ...
    if (SortLines_ShouldReverse())
        ...
        postProcessor = idendity;
}
#+END_SRC
**** Exercise 5-16
Turns out our design is not good enough to allow extensions; here try SOLID
design principle to improve our own design.

First, let us extend our own design without thinking about the SOLID principle,
and then we will review the SOLID principle and how it going to evaluate our own
result.

As usually, we start to code the tests:
#+BEGIN_SRC C
...
And this extension make the test pass:
#+BEGIN_SRC C
...
Well, actually I quite satisfied with current result. Let us delegate the
evaluation after next exercise.
**** Exercise 5-17
As usual, we add the behavior we want to the test:
#+BEGIN_SRC C
...
TEST(SortLines, CanSortMultiFieldUsingDifferentComparator)
{
  ...
  addFormatStringToFakeInput("Field1: c Field2: 4 Field3: xx\n");
...
Only to make this test pass, we implement the spaghetti code:
#+BEGIN_SRC C
void SortLines_ParseCommandline(int argc, char *argv[])
{
    int c;
    ...
    currentOption.isNumeric = currentOption.shouldReverse
        = currentOption.shouldFold = currentOption.isDirectory
    ...
    while (--argc > 0 && (*++argv)[0] == '-')
    {
        while (c = *++argv[0])
            switch (c) {
                case 'n':
                    currentOption.isNumeric = 1;
                    break;
                case 'r':
                    currentOption.shouldReverse = 1;
                    break;
                case 'f':
                    currentOption.shouldFold = 1;
                    break;
                case 'd':
                    currentOption.isDirectory = 1;
                    break;
                case 'k':
...
#+END_SRC
Since all the test are green (except the behavior test, of course), we are good
to refactor the codes. By the inspection, we can realize field option parsing is
related to the outer loop more intimately than inner unlike the other options;
so we can refactor as follows:
#+BEGIN_SRC C
void SortLines_ParseCommandline(int argc, char *argv[])
{
    int c;
...
Yet it is spaghetti code; nevertheless let's move on:
#+BEGIN_SRC C
...
Next is the more horrible thing, behavior implementation:
#+BEGIN_SRC C
TEST(SortLines, CanSortMultiFieldUsingDifferentComparator)
{
  addFormatStringToFakeInput("Field1: A Field2: 3 Field3: xx\n");
...
And finally, we just clean up only for the topmost abstraction, we got
#+BEGIN_SRC C
#include "SortLines.h"
#include "Readlines.h"
#include "QuickSort.h"
...
From here we are going to keep track of book's contents using starting points
from =SandBox/tests=. Before coding the test, we need to first have concrete
image about the model we are going to implement. In the book, they described
their idea using informal UML.

The advantage to using UML is it allows us to use test doubles; it is the idea
inherited from the OOP -- substitution rule. So here we use UML as abstract
model of our design.

Then we specify what we want from the test doubles via test:
#+BEGIN_SRC C
...
That is, via =FakeTimeService= ADT, we can manipulate the time as we want to
set; by using =LightControllerSpy= we want to intercept the parameters
=LightScheduler= called with.

We can see =LightControllerSpy= is doing exactly the same thing
=RuntimeError_stub= did in abstract manner. In black box approach, via
=FakeTimeService= we manipulate the indirect input to the =LightScheduler=; via
=LightControllerSpy= we inspect the indirect output from =LightScheduler=.

That is, we are controlling the dependencies involved with =LightScheduler= with
test doubles; and this controlling is accomplished via layer out the interface
from the implementation -- so that it can be swapped with test doubles in link time.

While it is good to remind ourselves the goal of test and to deduce the
specifications the test doubles should provide, this test is too burden to start
with.

As usual, it would be better to start with the most trivial case that would
fixdate the outermost interface:
#+BEGIN_SRC C
...
To make this test pass, we need to give the minimal definition of
=LightControllerSpy= but any of =LightScheduler=.

We could make the pass by hard-coding the above two functions; but since we know
these are the test doubles, let's take short-circuit a little bit: Let's make
these function as standalone ADT.

Before that, we need to define the interface =LightController='s implementation
should supply:
#+BEGIN_SRC C
...
The =MAX_LIGHTS= is determined by requirement.

Now test drive the spy:
#+BEGIN_SRC C
...
Then we can define the spy's interface:
#+BEGIN_SRC C
...
Undefined symbols for architecture x86_64:
  "_LightControllerSpy_GetLastId", referenced from:
      TEST_LightControllerSpy_Create_Test::testBody() in LightControllerSpyTest.o
  "_LightControllerSpy_GetLastState", referenced from:
      TEST_LightControllerSpy_Create_Test::testBody() in LightControllerSpyTest.o
  "_LightController_Create", referenced from:
      TEST_GROUP_CppUTestGroupLightControllerSpy::setup() in LightControllerSpyTest.o
  "_LightController_Destroy", referenced from:
      TEST_GROUP_CppUTestGroupLightControllerSpy::teardown() in LightControllerSpyTest.o
ld: symbol(s) not found for architecture x86_64
clang: error: linker command failed with exit code 1 (use -v to see invocation)
...
Now we need to resolve link error:
#+BEGIN_SRC C
...
Running SandBox_CppUTest_tests
.
tests/HomeAutomation/LightControllerSpyTest.cpp:24: error: Failure in TEST(LightControllerSpy, Create)
	LONGS_EQUAL(LIGHT_ID_UNKNOWN, LightControllerSpy_GetLastId()) failed
	expected <-1 (0xffffffffffffffff)>
	but was  < 0 (0x0)>

.
Errors (1 failures, 2 tests, 2 ran, 1 checks, 0 ignored, 0 filtered out, 1 ms)
...
Then what we want from the spy is to remember the information about last light
controlled:
#+BEGIN_SRC C
TEST(LightControllerSpy, RemeberTheLastLightIdControlled)
{
...
Linking SandBox_CppUTest_tests
Undefined symbols for architecture x86_64:
  "_LightController_On", referenced from:
      TEST_LightControllerSpy_RemeberTheLastLightIdControlled_Test::testBody() in LightControllerSpyTest.o
ld: symbol(s) not found for architecture x86_64
clang: error: linker command failed with exit code 1 (use -v to see invocation)
make[1]: [SandBox_CppUTest_tests] Error 1 (ignored)
Running SandBox_CppUTest_tests
...
make -i -f MakefileCppUTest.mk
compiling LightControllerSpy.c
tests/HomeAutomation/LightControllerSpy.c:26:29: error: unused parameter 'id' [-Werror,-Wunused-parameter]
void LightController_On(int id)
                             ^
1 errors generated.
make[1]: [objs/tests/HomeAutomation/LightControllerSpy.o] Error 1 (ignored)
Linking SandBox_CppUTest_tests
clang: error: no such file or directory: 'objs/tests/HomeAutomation/LightControllerSpy.o'
make[1]: [SandBox_CppUTest_tests] Error 1 (ignored)
Running SandBox_CppUTest_tests
/bin/sh: ./SandBox_CppUTest_tests: No such file or directory
make[1]: [all] Error 127 (ignored)
...
Running SandBox_CppUTest_tests
.
tests/HomeAutomation/LightControllerSpyTest.cpp:31: error: Failure in TEST(LightControllerSpy, RemeberTheLastLightIdControlled)
	LONGS_EQUAL(10, LightControllerSpy_GetLastId()) failed
	expected <10 (0xa)>
...
: Running SandBox_CppUTest_tests
: ...
: OK (3 tests, 3 ran, 4 checks, 0 ignored, 0 filtered out, 0 ms)
:
...
Running SandBox_CppUTest_tests
.
tests/HomeAutomation/LightControllerSpyTest.cpp:35: error: Failure in TEST(LightControllerSpy, RemeberTheLastLightIdControlled)
	LONGS_EQUAL(2, LightControllerSpy_GetLastId()) failed
	expected < 2 (0x2)>
	but was  <10 (0xa)>

..
Errors (1 failures, 3 tests, 3 ran, 5 checks, 0 ignored, 0 filtered out, 1 ms)
...
: Running SandBox_CppUTest_tests
: ...
: OK (3 tests, 3 ran, 6 checks, 0 ignored, 0 filtered out, 1 ms)
:

Now we can go back to the main stream:
#+BEGIN_SRC C
TEST(LightScheduler, NoChangeToLightsDuringInitialization)
{
    LONGS_EQUAL(LIGHT_ID_UNKNOWN, LightControllerSpy_GetLastId());
...
tests/HomeAutomation/LightSchedulerTest.cpp:21: error: Failure in TEST(LightScheduler, NoChangeToLightsDuringInitialization)
	LONGS_EQUAL(LIGHT_ID_UNKNOWN, LightControllerSpy_GetLastId()) failed
	expected <-1 (0xffffffffffffffff)>
	but was  < 0 (0x0)>
...
Now we need to initialize controller appropriately:
#+BEGIN_SRC C
...
: Running SandBox_CppUTest_tests
: ...
: OK (3 tests, 3 ran, 8 checks, 0 ignored, 0 filtered out, 0 ms)
:

Now we drive the other test double -- =FakeTimeService= via which we will
control the indirect input to the =LightScheduler=:
#+BEGIN_SRC C
...
As it fails the test since there is no =FakeTimeService= or even any of
=LightScheduler= data structure, it's time to implement the test double.

I myself found that in the text that it seems like the code of test doubles
implemented at once; is it the only exception for the TDD? No, I guess, it is
just grossed over as the author assumes the readers get used to the TDD cycle
implicitly.

So our mission would supply the gap between the complete code and the starting test.

To specify =FakeTimeService=, our current test case is not sufficient -- we need
another more specific test cases that leads to concise implementation of test
double at the same time describing the behavior of its. It would be more
appropriate to make its own standalone test suite -- like the =LightControllerSpy=.

Fortunately, as we have concrete image of design interface, we can start from
there. =TimeService= is data structure which manage current time state. Only via
=TimeService=, we can access and set the current time:
#+BEGIN_SRC C
...
This test leads to implement interface:
#+BEGIN_SRC C
typedef struct Time
{
    int minuteOfDay;
...
Then the minimum code to make the compiler happy:
#+BEGIN_SRC C
...
Then now implement the minimum code:
#+BEGIN_SRC C
void TimeService_GetTime(Time *time)
{
...
: Running SandBox_CppUTest_tests
: ....
: OK (4 tests, 4 ran, 10 checks, 0 ignored, 0 filtered out, 1 ms)
:

Now it's time to add the core test to the test double:
#+BEGIN_SRC C
...
This drives us to refine the =TimeService= interface:
#+BEGIN_SRC C
typedef enum {
...
Then it cascade changes to =FakeTimeService.h=:
#+BEGIN_SRC C
void FakeTimeService_SetDay(Day);
#+END_SRC

Now it leads implementation:
#+BEGIN_SRC C
...
make -i -f MakefileCppUTest.mk
compiling FakeTimeService.c
tests/HomeAutomation/FakeTimeService.c:9:17: error: integer constant not in range of enumerated type 'Day' [-Werror,-Wassign-enum]
    dayOfWeek = TIME_UNKNOWN;
                ^
tests/HomeAutomation/FakeTimeService.c:9:17: error: implicit conversion changes signedness: 'int' to 'Day' [-Werror,-Wsign-conversion]
    dayOfWeek = TIME_UNKNOWN;
              ~ ^~~~~~~~~~~~
2 errors generated.
make[1]: [objs/tests/HomeAutomation/FakeTimeService.o] Error 1 (ignored)
Linking SandBox_CppUTest_tests
clang: error: no such file or directory: 'objs/tests/HomeAutomation/FakeTimeService.o'
make[1]: [SandBox_CppUTest_tests] Error 1 (ignored)
Running SandBox_CppUTest_tests
....
OK (4 tests, 4 ran, 10 checks, 0 ignored, 0 filtered out, 1 ms)
...
Wow =enum= type works unlike the what I did in following experiment; I guess
it's due to the compilation option =-Wassign-enum=.

Anyway we need to fix this error. We shouldn't refactor the =int= type to the
=Day= at the first place until pass all the tests given. For now let us progress
#+BEGIN_SRC C
typedef enum {
NONE = -1,
SUNDAY=1, MONDAY, TUESDAY, WEDNESDAY, THURSDAY, FRIDAY, SATURDAY
...
TEST(FakeTimeService, Create)
{
    Time time;
    TimeService_GetTime(&time);
    LONGS_EQUAL(NONE, time.minuteOfDay);
...
: Running SandBox_CppUTest_tests
: .....
: OK (5 tests, 5 ran, 12 checks, 0 ignored, 0 filtered out, 1 ms)
:

Now we can make progress the main test, =LightScheduler=.
...
{
    bool test;
    printf("Uninitiazlied enum typed variable bound to %d\n", test);
    test = 5;
    printf("Out-of-range enum typed variable bound to %d\n", test);
}
#+END_SRC

#+RESULTS:
: Uninitiazlied enum typed variable bound to 1
: Out-of-range enum typed variable bound to 5
**** Make It Work for None, Then One
It's time to make pass following test. This test is /do nothing/; but it make
progress in concrete manner -- step by step:
#+BEGIN_SRC C
TEST(LightScheduler, NoScheduleNothingHappens)
{
    FakeTimeService_SetDay(MONDAY);
...
All we need to do to pass this test is to comply with compiler:
#+BEGIN_SRC C
...
: Running SandBox_CppUTest_tests
: ......
: OK (6 tests, 6 ran, 14 checks, 0 ignored, 0 filtered out, 1 ms)
:

Now move on next test, which was drove all the test doubles' specifications at
the first:
#+BEGIN_SRC C
TEST(LightScheduler, ScheduleOnEverydayNotTimeYet)
{
...
It leads to change in =LightScheduler.h=:
#+BEGIN_SRC C
#include "TimeService.h"
...
Then make the compiler happy:
#+BEGIN_SRC C
void LightScheduler_ScheduleTurnOn(int id, Day day, int minute)
{
    id = id + day + minute;     /* just to make the compiler happy */
}
#+END_SRC
...
make -i -f MakefileCppUTest.mk
compiling LightSchedulerTest.cpp
tests/HomeAutomation/LightSchedulerTest.cpp:43:38: error: use of undeclared identifier 'EVERYDAY'
    LightScheduler_ScheduleTurnOn(3, EVERYDAY, 1200);
                                     ^
1 error generated.
make[1]: [objs/tests/HomeAutomation/LightSchedulerTest.o] Error 1 (ignored)
compiling LightScheduler.c
Building archive lib/libSandBox_CppUTest.a
r - objs/src/LedDriver/LedDriver.o
r - objs/src/HomeAutomation/LightScheduler.o
Linking SandBox_CppUTest_tests
clang: error: no such file or directory: 'objs/tests/HomeAutomation/LightSchedulerTest.o'
make[1]: [SandBox_CppUTest_tests] Error 1 (ignored)
Running SandBox_CppUTest_tests
......
OK (6 tests, 6 ran, 14 checks, 0 ignored, 0 filtered out, 1 ms)
...
Need to include =EVERYDAY= to the =Day=:
#+BEGIN_SRC C
typedef enum {
NONE = -1, EVERYDAY = 10,
SUNDAY=1, MONDAY, TUESDAY, WEDNESDAY, THURSDAY, FRIDAY, SATURDAY
...
Running SandBox_CppUTest_tests
.......
OK (7 tests, 7 ran, 16 checks, 0 ignored, 0 filtered out, 0 ms)

#+end_example

Next, it's time to implement =TurnOn=:
#+BEGIN_SRC C
...
Make sure that this new test fails:
#+BEGIN_SRC sh :exports both :results verbatim
cd C_codes/TDD/code/SandBox
make codeCppUTest 2>&1
#+END_SRC
...
tests/HomeAutomation/LightSchedulerTest.cpp:61: error: Failure in TEST(LightScheduler, ScheduleOnEverydayItsTime)
	LONGS_EQUAL(3, LightControllerSpy_GetLastId()) failed
	expected < 3 (0x3)>
	but was  <-1 (0xffffffffffffffff)>

........
Errors (1 failures, 8 tests, 8 ran, 17 checks, 0 ignored, 0 filtered out, 1 ms)

make[1]: [all] Error 1 (ignored)
#+end_example

Let's make this test pass. We need to think what information =ScheduleTurnOn=
should possess, and how =Wakeup= should get the time information from
=TimeService= and then call =controller= appropriately.

Let us first code =ScheduleTurnOn=; it should remember given =id= and =minute=
to be used in =Wakeup=:
#+BEGIN_SRC C
static int scheduledLightId;
...
Since it is compiled as expected, now it's time to code =Wakeup=; it should get
the current time from =TimeService=, and then call the =LightController=
appropriately:
#+BEGIN_SRC C
void LightScheduler_Wakeup(void)
{
    ...
    if (time.minuteOfDay == scheduledMinute)
        LightController_On(scheduledLightId);
}
#+END_SRC
...
make -i -f MakefileCppUTest.mk
compiling LightScheduler.c
src/HomeAutomation/LightScheduler.c:25:48: warning: unused parameter 'day' [-Wunused-parameter]
void LightScheduler_ScheduleTurnOn(int id, Day day, int minute)
                                               ^
1 warning generated.
Building archive lib/libSandBox_CppUTest.a
...
Running SandBox_CppUTest_tests
........
OK (8 tests, 8 ran, 18 checks, 0 ignored, 0 filtered out, 0 ms)

#+end_example

Since all the test passes, it's time to refactor; =LightScheduler= should keep
track of multiple light events and the light event consists of light device id
and the scheduled time. However our current version does not have the light
event entity but rather have the information in component wise. We should treat
event in one entity:
#+BEGIN_SRC C
typedef struct
{
...
Since it compiled as expected, we replace the old discrete information with new
entity:
#+BEGIN_SRC C
void LightScheduler_Wakeup(void)
{
...
make -i -f MakefileCppUTest.mk
compiling LightScheduler.c
src/HomeAutomation/LightScheduler.c:30:48: warning: unused parameter 'day' [-Wunused-parameter]
void LightScheduler_ScheduleTurnOn(int id, Day day, int minute)
                                               ^
1 warning generated.
Building archive lib/libSandBox_CppUTest.a
...
Running SandBox_CppUTest_tests
........
OK (8 tests, 8 ran, 18 checks, 0 ignored, 0 filtered out, 1 ms)
...
Before that, let's refactor the conditional clause to use guard clause:
#+BEGIN_SRC C
void LightScheduler_Create(void)
{
...
make -i -f MakefileCppUTest.mk
compiling LightScheduler.c
src/HomeAutomation/LightScheduler.c:46:48: warning: unused parameter 'day' [-Wunused-parameter]
void LightScheduler_ScheduleTurnOn(int id, Day day, int minute)
                                               ^
src/HomeAutomation/LightScheduler.c:53:49: warning: unused parameter 'day' [-Wunused-parameter]
...
Now we can implement the new code:
#+BEGIN_SRC C
void LightScheduler_Wakeup(void)
{
    Time time;
...
: Running SandBox_CppUTest_tests
: .........
: OK (9 tests, 9 ran, 20 checks, 0 ignored, 0 filtered out, 1 ms)
:

Since we have detected duplication and all the test passed, it's time to
refactor the code:
#+BEGIN_SRC C
static void scheduleEvent(int id, Day day, int minuteOfDay, EventType event)
{
    scheduledEvent.id = id;
...
make -i -f MakefileCppUTest.mk
compiling LightScheduler.c
src/HomeAutomation/LightScheduler.c:48:39: warning: unused parameter 'day' [-Wunused-parameter]
static void scheduleEvent(int id, Day day, int minuteOfDay, EventType event)
                                      ^
1 warning generated.
Building archive lib/libSandBox_CppUTest.a
...
And =Wakeup= do not agree what we specified informally; it do the works way too
much breaking the abstraction layer. We need to make another helper function
that processes =scheduledEvent= appropriately:
#+BEGIN_SRC C
...
: Running SandBox_CppUTest_tests
: .........
: OK (9 tests, 9 ran, 20 checks, 0 ignored, 0 filtered out, 0 ms)
:

The if-else clause of =processEventDueNow= is not appropriate to its function's
role -- it supposed to process given light event with respect to given time:
#+BEGIN_SRC C
...
make -i -f MakefileCppUTest.mk
compiling LightScheduler.c
src/HomeAutomation/LightScheduler.c:58:39: warning: unused parameter 'day' [-Wunused-parameter]
static void scheduleEvent(int id, Day day, int minuteOfDay, EventType event)
                                      ^
1 warning generated.
Building archive lib/libSandBox_CppUTest.a
...
Time to refactor the test. The current version of test is hard to read:
#+BEGIN_SRC C
...
So far, we ignored the day -- we only tested =EVERYDAY= cases. It's time to
tackle the specific day matching cases:
#+BEGIN_SRC C
...
tests/HomeAutomation/LightSchedulerTest.cpp:79: error: Failure in TEST(LightScheduler, ScheduleTuesdayButItsMonday)
tests/HomeAutomation/LightSchedulerTest.cpp:32: error:
	LONGS_EQUAL(id, LightControllerSpy_GetLastId()) failed
	expected <-1 (0xffffffffffffffff)>
	but was  < 3 (0x3)>

..........
Errors (1 failures, 10 tests, 10 ran, 21 checks, 0 ignored, 0 filtered out, 1 ms)

make[1]: [all] Error 1 (ignored)
#+end_example

As the test is failed, it's time to code; the first step is hard code -- just
ignore the case where day is not =EVERYDAY=:
#+BEGIN_SRC C
typedef struct
{
    int id;
...
Running SandBox_CppUTest_tests
..........
OK (10 tests, 10 ran, 22 checks, 0 ignored, 0 filtered out, 1 ms)

#+end_example

The following test leads us to implement properly:
#+BEGIN_SRC C
...
static void processEventDueNow(Time *time, ScheduledLightEvent *lightEvent)
{
    ...
    Day scheduledDay = lightEvent->day;
...
Running SandBox_CppUTest_tests
...........
OK (11 tests, 11 ran, 24 checks, 0 ignored, 0 filtered out, 0 ms)

#+end_example

Now, following the test list,
#+BEGIN_SRC C
...
We need to change =Day= to include =WEEKEND=:
#+BEGIN_SRC C
typedef enum {
NONE = -1, EVERYDAY = 10, WEEKEND,
SUNDAY=1, MONDAY, TUESDAY, WEDNESDAY, THURSDAY, FRIDAY, SATURDAY
} Day;
#+END_SRC
...
: Running SandBox_CppUTest_tests
: ............
: OK (12 tests, 12 ran, 26 checks, 0 ignored, 0 filtered out, 1 ms)
:

And test passed as it is. The next test leads to modify the code:
#+BEGIN_SRC C
...
tests/HomeAutomation/LightSchedulerTest.cpp:109: error: Failure in TEST(LightScheduler, ScheduleWeekEndAndItsSaturday)
tests/HomeAutomation/LightSchedulerTest.cpp:32: error:
	LONGS_EQUAL(id, LightControllerSpy_GetLastId()) failed
	expected < 3 (0x3)>
	but was  <-1 (0xffffffffffffffff)>
...
.............
Errors (1 failures, 13 tests, 13 ran, 27 checks, 0 ignored, 0 filtered out, 1 ms)
...
static void processEventDueNow(Time *time, ScheduledLightEvent *lightEvent)
{
    ...
    if (scheduledDay != EVERYDAY &&
        scheduledDay != today &&
        !(scheduledDay == WEEKEND && today == SATURDAY))
        return;
    if (lightEvent->minuteOfDay != time->minuteOfDay)
        return;
    operateLight(lightEvent);
...
: Running SandBox_CppUTest_tests
: .............
: OK (13 tests, 13 ran, 28 checks, 0 ignored, 0 filtered out, 0 ms)
:
...
static void processEventDueNow(Time *time, ScheduledLightEvent *lightEvent)
{
    ...
    if (scheduledDay != EVERYDAY &&
        scheduledDay != today &&
        !(scheduledDay == WEEKEND && (today == SATURDAY || today == SUNDAY)))
        return;
    if (lightEvent->minuteOfDay != time->minuteOfDay)
        return;
    operateLight(lightEvent);
...
Running SandBox_CppUTest_tests
..............
OK (14 tests, 14 ran, 30 checks, 0 ignored, 0 filtered out, 1 ms)

#+end_example

Then final boundary case of scheduled at weekend:
#+BEGIN_SRC C
...
: Running SandBox_CppUTest_tests
: ...............
: OK (15 tests, 15 ran, 32 checks, 0 ignored, 0 filtered out, 1 ms)
:

Now it's time for weekday:
#+BEGIN_SRC C
TEST(LightScheduler, ScheduleWeekDayButItsSaturday)
{
...
Running SandBox_CppUTest_tests
................
OK (16 tests, 16 ran, 34 checks, 0 ignored, 0 filtered out, 1 ms)

#+end_example

#+BEGIN_SRC C
TEST(LightScheduler, ScheduleWeekDayButItsSunday)
{
    LightScheduler_ScheduleTurnOn(3, WEEKDAY, 1200);
    setTimeTo(SUNDAY, 1200);
...
: Running SandBox_CppUTest_tests
: .................
: OK (17 tests, 17 ran, 36 checks, 0 ignored, 0 filtered out, 0 ms)
:
...
tests/HomeAutomation/LightSchedulerTest.cpp:159: error: Failure in TEST(LightScheduler, ScheduleWeekDayAndItsMonday)
tests/HomeAutomation/LightSchedulerTest.cpp:32: error:
	LONGS_EQUAL(id, LightControllerSpy_GetLastId()) failed
	expected < 3 (0x3)>
	but was  <-1 (0xffffffffffffffff)>
...
..................
Errors (1 failures, 18 tests, 18 ran, 37 checks, 0 ignored, 0 filtered out, 1 ms)
...
static void processEventDueNow(Time *time, ScheduledLightEvent *lightEvent)
{
    ...
    if (scheduledDay != EVERYDAY &&
        scheduledDay != today &&
        !(scheduledDay == WEEKEND && (today == SATURDAY || today == SUNDAY)) &&
        !(scheduledDay == WEEKDAY && (today == MONDAY)))
        return;
    if (lightEvent->minuteOfDay != time->minuteOfDay)
        return;
    operateLight(lightEvent);
...
Running SandBox_CppUTest_tests
..................
OK (18 tests, 18 ran, 38 checks, 0 ignored, 0 filtered out, 1 ms)

#+end_example

And
#+BEGIN_SRC C
TEST(LightScheduler, ScheduleWeekDayAndItsFriday)
{
    LightScheduler_ScheduleTurnOn(3, WEEKDAY, 1200);
    setTimeTo(FRIDAY, 1200);
...
static void processEventDueNow(Time *time, ScheduledLightEvent *lightEvent)
{
    ...
    if (scheduledDay != EVERYDAY &&
        scheduledDay != today &&
        !(scheduledDay == WEEKEND && (today == SATURDAY || today == SUNDAY)) &&
        !(scheduledDay == WEEKDAY && (today >= MONDAY && today <= FRIDAY)))
        return;
    if (lightEvent->minuteOfDay != time->minuteOfDay)
        return;
    operateLight(lightEvent);
...
Running SandBox_CppUTest_tests
..................
OK (18 tests, 18 ran, 38 checks, 0 ignored, 0 filtered out, 0 ms)

#+end_example

Since all the tests passes, it's time to refactor the complicate conditional
expression into helper function to retain its abstract state:
#+BEGIN_SRC C
static BOOL ShouldLightRespondToday(Day today, Day scheduledDay)
{
    if (scheduledDay == EVERYDAY)
        return TRUE;
    ...
    if (scheduledDay == WEEKDAY)
        return today >= MONDAY && today <= FRIDAY;
    return FALSE;
}
...
static void processEventDueNow(Time *time, ScheduledLightEvent *lightEvent)
{
    ...
    if (lightEvent->id == UNUSED)
        return;
    if (!ShouldLightRespondToday(today, scheduledDay))
        return;
    if (lightEvent->minuteOfDay != time->minuteOfDay)
        return;
    operateLight(lightEvent);
...
Running SandBox_CppUTest_tests
...................
OK (19 tests, 19 ran, 40 checks, 0 ignored, 0 filtered out, 1 ms)

#+end_example

Now we test the connection between the CUT with the dependencies:
#+BEGIN_SRC C
...
And the minimum code comply the compiler:
#+BEGIN_SRC C
...
tests/HomeAutomation/LightSchedulerTest.cpp:187: error: Failure in TEST(LightSchedulerInitAndCleanup, CreateStartsOneMinuteAlarm)
	expected <0x10f87b140>
	but was  <0x0>

....................
Errors (1 failures, 20 tests, 20 ran, 41 checks, 0 ignored, 0 filtered out, 1 ms)

make[1]: [all] Error 1 (ignored)
#+end_example

Now it's time to code appropriately:
#+BEGIN_SRC C
void LightScheduler_Create(void)
{
    scheduledEvent.id = UNUSED;
...
And for the test double:
#+BEGIN_SRC C
static int registeredPeriod;
static WakeUpCallback registeredCallback;
...
void TimeService_SetPeriodicAlarmInSeconds(
    int seconds, WakeUpCallback callback)
{
    registeredPeriod = seconds;
...
: Running SandBox_CppUTest_tests
: ....................
: OK (20 tests, 20 ran, 42 checks, 0 ignored, 0 filtered out, 0 ms)
:

Following test drives to cancel the registered callback:
#+BEGIN_SRC C
...
tests/HomeAutomation/LightSchedulerTest.cpp:196: error: Failure in TEST(LightSchedulerInitAndCleanup, DestroyCancelsOneMinuteAlarm)
	expected <0x0>
	but was  <0x102000fe0>

.....................
Errors (1 failures, 21 tests, 21 ran, 43 checks, 0 ignored, 0 filtered out, 1 ms)
...
compiling FakeTimeServiceTest.cpp
compiling LightSchedulerTest.cpp
tests/HomeAutomation/LightSchedulerTest.cpp:186:20: warning: cast between pointer-to-function and pointer-to-object is an extension [-Wpedantic]
    POINTERS_EQUAL((void *)LightScheduler_Wakeup,
                   ^~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/Users/hyunngyu-jang/org/C_codes/cpputest/include/CppUTest/UtestMacros.h:262:30: note: expanded from macro 'POINTERS_EQUAL'
    POINTERS_EQUAL_LOCATION((expected), (actual), NULLPTR, __FILE__, __LINE__)
                             ^~~~~~~~
/Users/hyunngyu-jang/org/C_codes/cpputest/include/CppUTest/UtestMacros.h:268:68: note: expanded from macro 'POINTERS_EQUAL_LOCATION'
  do { UtestShell::getCurrent()->assertPointersEqual((const void *)expected, (const void *)actual, text, file, line); } while(0)
                                                                   ^~~~~~~~
tests/HomeAutomation/LightSchedulerTest.cpp:187:20: warning: cast between pointer-to-function and pointer-to-object is an extension [-Wpedantic]
                   (void *)FakeTimeService_GetAlarmCallBack());
                   ^~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/Users/hyunngyu-jang/org/C_codes/cpputest/include/CppUTest/UtestMacros.h:262:42: note: expanded from macro 'POINTERS_EQUAL'
    POINTERS_EQUAL_LOCATION((expected), (actual), NULLPTR, __FILE__, __LINE__)
                                         ^~~~~~
/Users/hyunngyu-jang/org/C_codes/cpputest/include/CppUTest/UtestMacros.h:268:92: note: expanded from macro 'POINTERS_EQUAL_LOCATION'
  do { UtestShell::getCurrent()->assertPointersEqual((const void *)expected, (const void *)actual, text, file, line); } while(0)
                                                                                           ^~~~~~
tests/HomeAutomation/LightSchedulerTest.cpp:196:26: warning: cast between pointer-to-function and pointer-to-object is an extension [-Wpedantic]
    POINTERS_EQUAL(NULL, (void *)FakeTimeService_GetAlarmCallBack());
                         ^~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/Users/hyunngyu-jang/org/C_codes/cpputest/include/CppUTest/UtestMacros.h:262:42: note: expanded from macro 'POINTERS_EQUAL'
    POINTERS_EQUAL_LOCATION((expected), (actual), NULLPTR, __FILE__, __LINE__)
...
compiling FakeTimeService.c
tests/HomeAutomation/FakeTimeService.c:32:9: warning: unused parameter 'seconds' [-Wunused-parameter]
    int seconds, WakeUpCallback callback)
        ^
tests/HomeAutomation/FakeTimeService.c:32:33: warning: unused parameter 'callback' [-Wunused-parameter]
    int seconds, WakeUpCallback callback)
                                ^
2 warnings generated.
compiling LightScheduler.c
...
Running SandBox_CppUTest_tests
.....................
OK (21 tests, 21 ran, 43 checks, 0 ignored, 0 filtered out, 0 ms)

#+end_example

Actually, our test doubles hard coded -- we need to test drive the correct
behavior in the =FakeTimeService= specific tests:
#+BEGIN_SRC C
TEST(FakeTimeService, SetAlarm)
{
    TimeService_SetPeriodicAlarmInSeconds(60, TestCallback1);
    POINTERS_EQUAL((void *)TestCallback1,
                   (void *)FakeTimeService_GetAlarmCallBack());
    LONGS_EQUAL(60, FakeTimeService_GetAlarmPeriod());
}

TEST(FakeTimeService, CancelAlarmSet)
{
    TimeService_SetPeriodicAlarmInSeconds(60, TestCallback1);
    POINTERS_EQUAL((void *)TestCallback1,
                   (void *)FakeTimeService_GetAlarmCallBack());
    LONGS_EQUAL(60, FakeTimeService_GetAlarmPeriod());
    TimeService_CancelPeriodicAlarmInSeconds(60, TestCallback1);
    POINTERS_EQUAL(NULL,
                   (void *)FakeTimeService_GetAlarmCallBack());
    LONGS_EQUAL(NONE, FakeTimeService_GetAlarmPeriod());
}

TEST(FakeTimeService, CancelAlarmUnset)
{
    TimeService_SetPeriodicAlarmInSeconds(60, TestCallback1);
    POINTERS_EQUAL((void *)TestCallback1,
                   (void *)FakeTimeService_GetAlarmCallBack());
    LONGS_EQUAL(60, FakeTimeService_GetAlarmPeriod());
    TimeService_CancelPeriodicAlarmInSeconds(60, TestCallback2);
    POINTERS_EQUAL((void *)TestCallback1,
                   (void *)FakeTimeService_GetAlarmCallBack());
    LONGS_EQUAL(60, FakeTimeService_GetAlarmPeriod());
}
...
Running SandBox_CppUTest_tests
...................
tests/HomeAutomation/FakeTimeServiceTest.cpp:76: error: Failure in TEST(FakeTimeService, CancelAlarmUnset)
	expected <0x10123d250>
	but was  <0x0>

.
tests/HomeAutomation/FakeTimeServiceTest.cpp:65: error: Failure in TEST(FakeTimeService, CancelAlarmSet)
	LONGS_EQUAL(NONE, FakeTimeService_GetAlarmPeriod()) failed
	expected <-1 (0xffffffffffffffff)>
	but was  <60 (0x3c)>

....
Errors (2 failures, 24 tests, 24 ran, 52 checks, 0 ignored, 0 filtered out, 1 ms)
...
void TimeService_CancelPeriodicAlarmInSeconds(
    int seconds, WakeUpCallback callback)
{
    if (registeredPeriod != seconds)
        return;
    if (registeredCallback != callback)
        return;
    registeredPeriod = NONE;
    registeredCallback = NULL;
}
#+END_SRC
...
: Running SandBox_CppUTest_tests
: ........................
: OK (24 tests, 24 ran, 53 checks, 0 ignored, 0 filtered out, 1 ms)
:

And now all the tests are passed, let's refactor the test doubles:
#+BEGIN_SRC C
...
:NOTER_PAGE: 170
:END:
As usual, let's test drive the "many" case:
#+BEGIN_SRC C
...
Running SandBox_CppUTest_tests
.................
tests/HomeAutomation/LightControllerSpyTest.cpp:45: error: Failure in TEST(LightControllerSpy, RememberAllLightStates)
	LONGS_EQUAL(LIGHT_OFF, LightControllerSpy_GetLightState(31)) failed
	expected <0 (0x0)>
	but was  <1 (0x1)>

........
Errors (1 failures, 25 tests, 25 ran, 55 checks, 0 ignored, 0 filtered out, 1 ms)

make[1]: [all] Error 1 (ignored)
#+end_example

And now, the minimum code to pass the test:
#+BEGIN_SRC C
...
: Running SandBox_CppUTest_tests
: .........................
: OK (25 tests, 25 ran, 55 checks, 0 ignored, 0 filtered out, 1 ms)
:

As all tests are green, it's time to refactor:
#+BEGIN_SRC C
    void checkLightState(int id, int level)
    {
...
As our controller spy updated its feature upto multiple light handling, we can
drove the scheduler as same as that:
#+BEGIN_SRC C
...
We should be careful about the transition from single case into multiple case
to ensure "Don't burn your bridges."

Since we have made abstract layers in our implementation using ADT, to support
new feature, only small portion of existing functions needs modifications:
...
Finally we need to update the callback function:
#+BEGIN_SRC C
void LightScheduler_Wakeup(void)
{
    int i;
...
Now all the tests pass; now our old bridge can be torched.
...
We needed to break the loop whenever found the blank slot.

Next is the overflow check:
#+BEGIN_SRC C
...
This leads to following modification.
#+BEGIN_SRC C
static int scheduleEvent(int id, Day day, int minuteOfDay, EventType event)
{
    int i;
...
The test proceed by the removing case:
#+BEGIN_SRC C
TEST(LightScheduler, RemoveRecyclesScheduleSlot)
{
    ...
    LONGS_EQUAL(LS_OK,
                LightScheduler_ScheduleTurnOn(3, MONDAY, 1000));
}
#+END_SRC

And this leads more specific remove test:
#+BEGIN_SRC C
...
And finally the test caused all the subsequent remove cases:
#+BEGIN_SRC C
TEST(LightScheduler, RemoveRecyclesScheduleSlot)
{
    int i;
...
Lastly, we test the out-of-bound case:
#+BEGIN_SRC C
...
Here we going to study more granular method to control test double, using
function pointer.

Let's assume that we want to add randomness to our light scheduler. Now we need
to test drive this new feature. At first, we need to write test that check
whether the new feature works as expected. But what the sense is "expected" when
you dealing with randomness? We can not drove the test current organization; we
need to control the "random" number so that we can set the /expected/ random
number and then check whether /expected/ number is reflected in scheduler.

It sounds we need to stub the randomization process the scheduler should depend
to test that feature; we also need to implement random number generator, which
means we also need to test drive the generator itself.

So, /by reasoning/, we broke down the new task into two different task -- one
for stubbing the random number generator for test scheduler and one for
random number generator itself to be used in production code -- yet we want to
test those test codes in same compilation unit.

That means, our old link substitution method for test double would not work in
this scheme.

**** Testing Randomness
Let's tackle the easy one first: Random number generator.
#+BEGIN_SRC C
TEST(RandomMinute, GetIsInRange)
{
  for (int i = 0; i < 100; i++)
      AssertMinuteIsInRange(RandomMinute_Get());
}
#+END_SRC

This test specifies what we want: We want to assure the numbers =RandomMinute=
spits in the expected range.

And the following code setup the test appropriately:
#+BEGIN_SRC C
TEST_GROUP(RandomMinute)
{
  ...
  void AssertMinuteIsInRange(int minute)
  {
...
And here is the minimum code to pass test:
#+BEGIN_SRC C
static int bound = 0;
...
We'd better to ensure repeatable property not to fail due to randomness:
#+BEGIN_SRC C
  void setup()
  {
    RandomMinute_Create(BOUND);
...
Next is to ensure that all the values in the range can be obtained:
#+BEGIN_SRC C
...
As noted before, we need to stub the random number generator with test double;
however our current generator fixed at link time.

Let us first specify what we want:
#+BEGIN_SRC C
...
The =FakeRandomMinute_SetFirstAndIncrement(-10, 5)= make the =RandomMinute_Get=
to spit first -10 and then -5, 0, 5, ...; that is set the start number to -10
and then increments by 5.

As we designed what we want from the =FakeRandomMinute=, it's time to refactor
the generator to be flexible enough to allow to be stubbed.
#+BEGIN_SRC C
void RandomMinute_Create(int);
extern int (*RandomMinute_Get)(void);
#+END_SRC

Here we declared the function pointer rather than function prototype. And we
define that variable in the implementation:
#+BEGIN_SRC C
...
Via this, we can stub the implementation in runtime; not the compilation time:
#+BEGIN_SRC C
...
Now test drive the =FakeRandomMinute=:
#+BEGIN_SRC C
TEST(FakeRandomMinute, SetFirstAndIncrementThenGet)
{
    int i;
    int start = -10;
    int increment = 5;

    FakeRandomMinute_SetFirstAndIncrement(start, increment);

    for (i = 0; i < 20; ++i)
        LONGS_EQUAL(start + increment * i, FakeRandomMinute_Get());
}
#+END_SRC
...
Now we return to the mainstream:
#+BEGIN_SRC C
void LightScheduler_Randomize(int id, Day day, int minute)
{
    ...
    for (i = 0; i < MAX_EVENTS; i++)
    {
        if (scheduledEvents[i].id == id &&
            ...
            scheduledEvents[i].minuteOfDay == minute)
        {
            scheduledEvents[i].minuteOfDay = minute + RandomMinute_Get();
        }
    }
}
#+END_SRC
...
Here we try to make the printed output to be checked automatically.

First, let us assume that we have =printf= like function =FormatOutput= and its
prototype is as follows:
#+BEGIN_SRC C
int FormatOutput(const char *, ...);
#+END_SRC

And we want to stub this function only in test codes; it calls the function
pointer stubbing:
#+BEGIN_SRC C
extern int (*FormatOutput)(const char *, ...);
#+END_SRC

Then our previous implementation got
#+BEGIN_SRC C
...
And now we specify what the spy should behave.

First we can retrieve what the printed using this spy like this:
#+BEGIN_SRC C
TEST(FormatOutputSpy, HelloWorld)
{
  FormatOutput_Create(20);
  FormatOutput("Hello, World\n");
  STRCMP_EQUAL("Hello, World\n", FormatOutputSpy_GetOutput());
}
#+END_SRC

And, as it has the limit buffer size, it only can store until the buffer full:
#+BEGIN_SRC C
TEST(FormatOutputSpy, LimitTheOutputBufferSize)
{
  FormatOutputSpy_Create(4);
...
Finally, we can retrieve the printed output which constituted via multiple
...
And finally, the spy's limit does not depend on the total times it has been
called:
#+BEGIN_SRC C
TEST(FormatOutputSpy, PrintMultipleOutputsPastFull)
{
...
Now we implement those behaviors:
#+BEGIN_SRC C
...
Here we are going to use the newly created spy, =FormatOutputSpy= in the case of
=CircularBuffer=. Let's test drive =CircularBuffer_Print=.

The first step is, as we have learned, to test none case:
#+BEGIN_SRC C
...
It contains all the things we want; the constitute the fixture, the output
format, and the structure of function. The natural step that we should take
first is to construct the fixture:
#+BEGIN_SRC C
...
Since our =FormatOutputSpy_GetOutput= returns the pointer to the stored buffer,
we don't need to update =actualOutput= whenever we want to peak the contents.

Now let's the none case pass:
#+BEGIN_SRC C
void CircularBuffer_Print(CircularBuffer buf)
{
    FormatOutput("Circular buffer content:\n<>\n");
}
#+END_SRC

Next is the 1 of 0-1-N pattern:
#+BEGIN_SRC C
TEST(CircularBufferPrint, PrintAfterOneIsPut)
{
    expectedOutput = "Circular buffer content:\n<17>\n";

    CircularBuffer_Push(17, buffer);
    CircularBuffer_Print(buffer);
...
void CircularBuffer_Print(CircularBuffer buf)
{
    FormatOutput("Circular buffer content:\n<");
    if (!CircularBuffer_IsEmpty(buf))
        FormatOutput("%d", *buf->frontp);
    FormatOutput(">\n");
}
#+END_SRC

Next is N case but in the most simplest form:
#+BEGIN_SRC C
TEST(CircularBufferPrint, PrintNotYetWrappedOrFull)
{
    expectedOutput = "Circular buffer content:\n<10, 20, 30>\n";

    CircularBuffer_Push(10, buffer);
    CircularBuffer_Push(20, buffer);
    CircularBuffer_Push(30, buffer);
    CircularBuffer_Print(buffer);
...
And it leads the code change as
#+BEGIN_SRC C
void CircularBuffer_Print(CircularBuffer buf)
{
    int *ip, size;
    ...
    for (ip = buf->frontp, size = buf->nelem; size-- > 0; ++ip)
        FormatOutput(size == 0 ? "%d" : "%d, ", *ip);
    FormatOutput(">\n");
}
#+END_SRC

Here is the next complexity, full case:
#+BEGIN_SRC C
TEST(CircularBufferPrint, PrintNotYetWrappedAndIsFull)
{
    ...
    CircularBuffer_Push(31, b);
    CircularBuffer_Push(41, b);
    CircularBuffer_Push(59, b);
    CircularBuffer_Push(26, b);
    CircularBuffer_Push(53, b);
...
Let's refactor our code to use the existing code rather than do the duplicate
things:
#+BEGIN_SRC C
void CircularBuffer_Print(CircularBuffer buf)
{
    ...
    while (!CircularBuffer_IsEmpty(buf))
        FormatOutput(buf->nelem == 1 ? "%d" : "%d, ", CircularBuffer_Pop(buf));
...
Since this make all the test green, let's do the further step; that is to
retain the abstractness level in the loop:
#+BEGIN_SRC C
static BOOL IsLast(CircularBuffer buf)
{
    return buf->nelem == 1;
}
...
void CircularBuffer_Print(CircularBuffer buf)
{
    ...
    while (!CircularBuffer_IsEmpty(buf))
        FormatOutput(IsLast(buf) ? "%d" : "%d, ", CircularBuffer_Pop(buf));
...
Now we finally test the wrapping case:
#+BEGIN_SRC C
TEST(CircularBufferPrint, PrintOldToNewWhenWrappedAndFull)
{
    ...
    CircularBuffer b = CircularBuffer_Create(5);
    CircularBuffer_Push(200, b);
    CircularBuffer_Push(201, b);
    CircularBuffer_Push(202, b);
    CircularBuffer_Push(203, b);
    CircularBuffer_Push(204, b);
    CircularBuffer_Pop(b);
    CircularBuffer_Push(999, b);
    CircularBuffer_Print(b);
...
Lastly we ensure printing do not cause any change in the contents of buffer:
#+BEGIN_SRC C
TEST(CircularBufferPrint, PrintingDoesNotHarmContents)
{
    expectedOutput =
        "Circular buffer content:\n<31, 41, 59, 26, 53>\n"
        "Circular buffer content:\n<31, 41, 59, 26, 1>\n";
...
#+END_SRC
*** The Mock Object
Now we will implement the more complicated test double, which we can control
over in the test code -- expect this and that and exercises this call and that
call finally checks the actual calls.

For illustration, this book, took the flash driver as example. Since it is
obviously hardware dependent object or more specifically it is the
bridge between the software world and the flash hardware memory.

The purpose is also obvious, if we can stub out the flash hardware part to
develop flash driver, then we can stub out anyother we want to do.
**** Flash Driver
We will stub the hardware dependent part, I/O part. Let's dive into the code. We
want to be able to use mock with following behavior:
#+BEGIN_SRC C
...
Then this is the object test case. Now it was supposed to turn to test drive
=MockIO=; however as you can see from above desired behavior, =MockIO= itself
use functions from the test harnesses. We are stuck; we want to test the code
that test given situation. We can think of this as /meta/ testing! So the
actual code implemented by the author do the mysterious things seen by us.

Without any test driving process to code the test double, we will get stuck to
the traditional debugging after programming.

For now we assume that we have such test double so that we can test the =Flash=
driver as we want; but we should think what the consequences of this. That is
you and I know that we don't have such mock yet in real world situation, we
don't have any author guiding us to the /right/ process to implement the task.

We can think of this process of implementing the test of =Flash= is also part of
wishful thinking we got used to. That is, we should wish at least vaguely to
start with what we need to implement the final goal; and then construct the
final production using the rough specifications. The rough specifications
evolves to be more specific while we use them in the production code as our
test list got evolved from the first sketch to the final test code.

Although the mock object used in the text book is already completed one, we
should think the changes it would experience through the code evolving.

***** Implementing =MockIO=
What should we expect from the =MockIO= from the sketch point? Having quite
concrete specification for its role and the interface it would implement, we
expect =MockIO= implement
#+BEGIN_SRC C
...
#+END_SRC
to be used as =IO= module in the test code. And the gist code of mock is that it
can be programmed in test code:
#+BEGIN_SRC C
MockIO_Expect_Write(0x1000, 0xBEEF);
MockIO_Expect_ReadThenReturn(0, 1<<7);
#+END_SRC

By pondering the gap between these desired core feature and the interface it
should replace, we came to realize that implementation code of interface should
check the expected behavior; we can assure that by thinking the use case as
designed roughly:
#+BEGIN_SRC C
  int result = 0;
  MockIO_Expect_Write(0, 0x40);
...
Finally we naturally came to think the counterpart of constructing expectations,
the exhaustion check:
#+BEGIN_SRC C
  MockIO_Verify_Complete();
#+END_SRC

These all the skeletal code we want from the mock object in this specific situation.
We will add the detailed specification of =MockIO= as we go through the test
code of =Flash= driver.

Back to the test of =Flash= driver, first we consider the test list:
...
Then as noted before, as one of successful case, we can count
#+BEGIN_SRC C
TEST(Flash, WriteSucceedsReadyImmediately)
{
  int result = 0;
...
We will study further with this test case to deduce the detailed specifications
of =MockIO=. Written in the text book, the =Expect= establishes expectations
phase of the test, first instruction puts the device in program mode. And the
first =Expect_Read...= singals to the driver the process successfully completed.
And the final expectation returns the written data to the driver confirming it
was written as expected.

Then the code under the test, =Flash_Write= is called with address and data. In
turns it calls the =IO_Read()= and =IO_Write= to accomplish given task; the mock
in turn checks the given requests from =Flash_Write= whether it meets preset
expectations in order.

Finally it checks whether the expectations exhausted as expected. Enough words
to specify informal behavior for above test code; it's time to look how the
=MockIO= responds to the unexpected inputs as we test drove the flash driver:
#+BEGIN_SRC C
...
Then what should we get from the execution of test code? Let's be in the
position of =MockIO=. Since =Flash_Write= does not call any of the =IO=
functions, we expect the first request after establishing expectations is
=MockIO_Verify_Complete()=. As it got 0 request while it expected 4 request as
whole, we expect from the =MockIO= to produce following error message:

#+BEGIN_EXAMPLE
mocks/MockIO.c:... error: Failure in TEST(Flash, WriteSucceedsReadyImmediately)
		Expected 4 reads/writes but got 0
#+END_EXAMPLE

Now if it meet the first expectation but with swapped parameters:
#+BEGIN_SRC C
int Flash_Write(ioAddress address, ioData data)
{
    IO_Write(0x40, 0);
...
This time we want to =MockIO= to complant as follows:
#+BEGIN_EXAMPLE
mocks/MockIO.c:... error: Failure in TEST(Flash, WriteSucceedsReadyImmediately)
		R/W 1: Expected IO_Write(0x0, 0x40)
        		But was IO_Write(0x40, 0x0)
#+END_EXAMPLE

This is part of the whole behavior constituting =MockIO=; we could keep doing
simulation until we drove all the failure modes. However it got quite concrete
what the rest are, we list those here:
- Checking for unconsumed expectations (=MockIO_Verify_Complete()=).
- Checking for wrong parameters (above case).
- Checking for wrong function call, such as =IO_Read()= called when expecting
  =IO_Write()=.
- Checking for wrong function call, such as Any =IO= R/W function called when
  expecting nothing.


The text progress with the test driving flash driver; but we can not do real job
unless we implement the =MockIO= rather than simulating. To do that, here we
list the test list for the =MockIO=:
- =MockIO= created with no expectations.
- =MockIO= complaints when there is no more expections but called with =IO=
  operations.
- =MockIO= ignore when given one expectation and that expectation matched with
  calling.
- =MockIO= complaints when given one (or more) expectation and called that
  operation but with wrong parameters -- one for =IO_Read= and one for
  =IO_Write=.
- =MockIO= complaints when given one (or more) expectation but called another
  operation.
- =MockIO= complaints when given one (or more) expectation and called nothing.
- =MockIO= ignore until given expections fullfilled with right order successive
  of calling.


As noted before here we performs meta testing, we need to manipulate meta
testing fixture:
#+BEGIN_SRC C
extern "C"
{
...
Then let's test the start point:
#+BEGIN_SRC C
TEST(MockIO, NoExpectationButCalled)
{
  FAIL("Start here");
}
#+END_SRC
...
{
    FAIL_TEXT_C("No more expectations but was IO_Read(0x10000)");
    return 0x0;
}
#+END_SRC
...
{
    FAIL_TEXT_C("No more expectations but was IO_Write(0x10000, 0x1234)");
}
#+END_SRC

Now let's get rid of the hard coding:
#+BEGIN_SRC C
static void TooManyRead1()
{
  IO_Read(0x10000);
}

static void TooManyRead2()
{
  IO_Read(0x20);
}
...
TEST(MockIO, TooManyReads)
{
  testFailureWith(TooManyRead1);
  fixture->assertPrintContains("No more expectations but was IO_Read(0x10000)");

  ++expectedErrors;

  testFailureWith(TooManyRead2);
  fixture->assertPrintContains("No more expectations but was IO_Read(0x10)");
}
#+END_SRC
...
ioData IO_Read(ioAddress offset)
{
    ...
    snprintf(message, size, "No more expectations but was IO_Read(0x%x)", offset);
    FAIL_TEXT_C(message);
    return 0x0;
...
And also
#+BEGIN_SRC C
static void TooManyWrite1()
{
    IO_Write(0x10000, 0x1234);
}

static void TooManyWrite2()
{
    IO_Write(0x100, 0x12);
}
...
TEST(MockIO, TooManyWrites)
{
  testFailureWith(TooManyWrite1);
  fixture->assertPrintContains("No more expectations but was IO_Write(0x10000, 0x1234)");

  ++expectedErrors;

  testFailureWith(TooManyWrite2);
  fixture->assertPrintContains("No more expectations but was IO_Write(0x100, 0x12)");
}
#+END_SRC

This leads for the write case,
#+BEGIN_SRC C
void IO_Write(ioAddress offset, ioData dat)
{
    char message[100];
    int size = sizeof message - 1;

    snprintf(message, size, "No more expectations but was IO_Write(0x%x, 0x%x)",
             offset, dat);
    FAIL_TEXT_C(message);
}
#+END_SRC
Since all the tests written so far are green, now good to refactor the code:
#+BEGIN_SRC C
static ioAddress addr;
static ioData value;
...
static void failWhenNoUnusedExpectations(char *format)
{
    ...
    snprintf(message, size, format, addr, value);
    FAIL_TEXT_C(message);
}
...
{
    addr = offset;
    failWhenNoUnusedExpectations("No more expectations but was IO_Read(0x%x)");
    return 0x0;
}
...
void IO_Write(ioAddress offset, ioData data)
{
    addr = offset;
    value = data;
    failWhenNoUnusedExpectations("No more expectations but was IO_Write(0x%x, 0x%x)");
}
#+END_SRC

Now we move on to 1 case.
#+BEGIN_SRC C
static void CanMatchOneWrite()
{
  MockIO_Expect_Write(1,2);
  IO_Write(1, 2);
}

TEST(MockIO, CanMatchOneWrite)
{
  expectedErrors = 0;
  testFailureWith(CanMatchOneWrite);
  fixture->assertPrintContains("OK");
}
#+END_SRC

It just pass without modification. The read and return is next:
#+BEGIN_SRC C
static void CanMatchOneReadAndReturn()
{
  MockIO_Expect_ReadThenReturn(1,2);
  LONGS_EQUAL(2, IO_Read(1));
}

TEST(MockIO, CanMatchOneReadAndReturn)
{
  expectedErrors = 0;
  testFailureWith(CanMatchOneReadAndReturn);
  fixture->assertPrintContains("OK");
}
#+END_SRC
...
Next we test the unmatched operation cases:
#+BEGIN_SRC C
...
And it leads hard coding like
#+BEGIN_SRC C
void IO_Write(ioAddress offset, ioData data)
{
    if (kind == IO_READ)
    {
        FAIL_TEXT_C("Expected IO_Read(0x0) would return 0x1\n"
...
ioData IO_Read(ioAddress offset)
{
    if (kind == IO_WRITE)
    {
        FAIL_TEXT_C("Expected IO_Write(0x0, 0x1)"
                    "But was IO_Read(0x0)");
        return 0x0;
    }
    addr = offset;
...
The mismatched parameters case is the next; this would force us to get rid of
all the hard coded portions:
#+BEGIN_SRC C
...
We need to refactor existing code as follows:
#+BEGIN_SRC C
...
Since this entity is common to both of =IO= operations.

And we need to change to prepend =expected= to the bare members of
=Expectation=. Then make sure the existing tests all pass but the newly added
one.

Now we are good to implement code to pass the newly added one:
#+BEGIN_SRC C
static Expectation actual;

void IO_Write(ioAddress offset, ioData data)
{
    if (expected.kind == IO_READ)
    {
        FAIL_TEXT_C("Expected IO_Read(0x0) would return 0x1\n"
                    "But was IO_Write(0x0, 0x0)");
        return;
    ...
    actual.addr = offset, actual.value = data;

    if (expected.kind == IO_UNSET)
    {
        failWhenNoUnusedExpectations("No more expectations but was IO_Write(0x%x, 0x%x)");
        return;
    }

    if (expected.addr != offset || expected.value != data)
    {
        char message[100];
        int size = sizeof message - 1;
        const char *format =
...
#+END_SRC
As implementing above, we were able to detect the bug hidden in
...
static void failWhenNoUnusedExpectations(const char *format)
{
    ...
    snprintf(message, size, format, actual.addr, actual.value);
    FAIL_TEXT_C(message);
}
#+END_SRC

And next test passes as it is:
#+BEGIN_SRC C
...
Now we like to get rid of the hard coded part:
#+BEGIN_SRC C
static void WriteWhenReadExpectedFail1()
{
  MockIO_Expect_ReadThenReturn(0,1);
  IO_Write(0,0);
}

static void WriteWhenReadExpectedFail2()
{
  MockIO_Expect_ReadThenReturn(0x23,1);
  IO_Write(0x2,0);
}

TEST(MockIO, WriteWhenReadExpectedFails)
{
  testFailureWith(WriteWhenReadExpectedFail1);
  fixture->assertPrintContains("Expected IO_Read(0x0) would return 0x1");
  fixture->assertPrintContains("But was IO_Write(0x0, 0x0)");

  ++expectedErrors;

  testFailureWith(WriteWhenReadExpectedFail2);
  fixture->assertPrintContains("Expected IO_Read(0x23) would return 0x1");
  fixture->assertPrintContains("But was IO_Write(0x2, 0x0)");
}
#+END_SRC
...
And
#+BEGIN_SRC C
static void ReadWhenWriteExpectedFail1()
{
    MockIO_Expect_Write(0,1);
    IO_Read(0);
}

static void ReadWhenWriteExpectedFail2()
{
    MockIO_Expect_Write(0x21,0x3);
    IO_Read(0x5);
}

TEST(MockIO, ReadWhenWriteExpectedFails)
{
  testFailureWith(ReadWhenWriteExpectedFail1);
  fixture->assertPrintContains("Expected IO_Write(0x0, 0x1)");
  fixture->assertPrintContains("But was IO_Read(0x0)");

  ++expectedErrors;

  testFailureWith(ReadWhenWriteExpectedFail2);
  fixture->assertPrintContains("Expected IO_Write(0x21, 0x3)");
  fixture->assertPrintContains("But was IO_Read(0x5)");
}
#+END_SRC
...
As all of the tests got passed, it's time to refactor it:
#+BEGIN_SRC C
...
static void failExpectation(char * expectationFailMessage)
{
    char message[100];
    int size = sizeof message - 1;
    snprintf(message, size, expectationFailMessage,
             expected.addr, expected.value,
             actual.addr, actual.value);
    FAIL_TEXT_C(message);
}
...
ioData IO_Read(ioAddress offset)
{
    ...
    if (expected.kind == IO_WRITE)
        failExpectation(report_expect_write_was_read);
...
void IO_Write(ioAddress offset, ioData data)
{
    ...
    if (expected.kind == IO_READ)
        failExpectation(report_expect_read_was_write);

    if (expected.kind == IO_UNSET)
        failWhenNoUnusedExpectations(report_write_but_out_of_expectations);

    if (expected.addr != offset || expected.value != data)
        failExpectation(report_write_does_not_match);
}
#+END_SRC

Now we check the exhaustion of expectation:
#+BEGIN_SRC C
...
TEST(MockIO, NotAllExpectationsUsed)
{
  ...
  fixture->assertPrintContains("Expected read/write but got nothing");
}
#+END_SRC

The following code make the test pass:
#+BEGIN_SRC C
void MockIO_Verify_Complete(void)
{
    if (expected.kind != IO_UNSET)
        FAIL_TEXT_C("Expected read/write but got nothing");
}
#+END_SRC

Now we need to move on N case:
#+BEGIN_SRC C
static void CanMatchSeveral()
{
  MockIO_Expect_Write(1, 2);
  MockIO_Expect_ReadThenReturn(3,1);
  IO_Write(1, 2);
  LONGS_EQUAL(1, IO_Read(3));
}

TEST(MockIO, CanMatchSeveral)
{
  expectedErrors = 0;
  testFailureWith(CanMatchSeveral);
  fixture->assertPrintContains("OK");
}
#+END_SRC
It forces us to change current structure to allocate dynamically since we want
to make the user (we!) specify the maximum expectations. So rather than current
test case, it would be better to check the boundary cases first:
#+BEGIN_SRC C
...
{
    expected.kind = IO_WRITE;
    expected.addr = offset;
    expected.value = data;

    if (setExpectationCount >= maxExpectationCount)
        FAIL_TEXT_C("MockIO_Expect_IO_Write: Too many expectations");

    expectations[setExpectationCount].kind = IO_WRITE;
    expectations[setExpectationCount].addr = offset;
    expectations[setExpectationCount].value = data;
    ++setExpectationCount;
}
#+END_SRC

For read is following:
#+BEGIN_SRC C
...
{
    expected.kind = IO_READ;
    expected.addr = offset;
    expected.value = data;

    if (setExpectationCount >= maxExpectationCount)
        FAIL_TEXT_C("MockIO_Expect_IO_Read: Too many expectations");

    expectations[setExpectationCount].kind = IO_Read;
    expectations[setExpectationCount].addr = offset;
    expectations[setExpectationCount].value = data;
    ++setExpectationCount;
}
...
We also test yet another underflow case:
#+BEGIN_SRC C
...
Now it's time to return the main stream:
#+BEGIN_SRC C
static void CanMatchSeveral()
{
  MockIO_Expect_Write(1, 2);
...
ioData IO_Read(ioAddress offset)
{
    ...
    expected = expectations[getExpectationCount++];
...
Now we can safely remove =IO_UNSET= in creation:
#+BEGIN_SRC C
void MockIO_Create(int maxExpectations)
{
    expectations = calloc(maxExpectations, sizeof (Expectation));
...
{
    if (expectations == 0)
        FAIL_TEXT_C(report_MockIO_not_initialized);
}
...
void MockIO_Expect_Write(ioAddress offset, ioData data)
{
    ...
    recordExpectation(IO_WRITE, offset, data);
}

void MockIO_Expect_ReadThenReturn(ioAddress offset, ioData data)
{
    failWhenNoRoomForExpectations(report_too_many_read_expectations);
    recordExpectation(IO_READ, offset, data);
}
#+END_SRC

And our current =IO_Read= and =IO_Write= does not recover from the error. We can
easily fix this by
#+BEGIN_SRC C
static void WriteWhenReadExpectedFail1()
{
...
static void WriteWhenReadExpectedRecover1()
{
  LONGS_EQUAL(1, IO_Read(0));
}
...
TEST(MockIO, WriteWhenReadExpectedFails)
{
  ...
  testFailureWith(WriteWhenReadExpectedRecover1);
  ++expectedErrors;
...
#+END_SRC
the counterpart test case can be implemented analogous to above. And
with
#+BEGIN_SRC C
ioData IO_Read(ioAddress offset)
{
...
ioData IO_Read(ioAddress offset)
{
    setExpectedAndActual(offset, IO_UNSET);
    failWhenNotInitialized();
    failWhenNoUnusedExpectations(report_read_but_out_of_expectations);
    failWhen(expectationIsNot(IO_READ), report_expect_write_was_read);
    failWhen(expectedAddressIsNot(offset), report_read_wrong_address);
...
void IO_Write(ioAddress offset, ioData data)
{
    ...
    failWhenNotInitialized();
    failWhenNoUnusedExpectations(report_write_but_out_of_expectations);
    failWhen(expectationIsNot(IO_WRITE), report_expect_read_was_write);
    failWhen(expectedAddressIsNot(offset), report_write_does_not_match);
...
Now we move to the final test case:
#+BEGIN_SRC C
static void MatchSeveralThenFail()
{
  MockIO_Expect_ReadThenReturn(3,1);
  MockIO_Expect_Write(0x0, 0x0);
  LONGS_EQUAL(1, IO_Read(3));
  IO_Read(0xdead);
}

TEST(MockIO, MatchSeveralThenFail)
{
  testFailureWith(MatchSeveralThenFail);
  fixture->assertPrintContains("R/W 2: Expected IO_Write(0x0, 0x0)");
  fixture->assertPrintContains("But was IO_Read(0xdead)");
}
#+END_SRC
...
We also update the =Verify_Complete=:
#+BEGIN_SRC C
static void NotAllExpectationsUsed()
{
    MockIO_Expect_ReadThenReturn(0x1000,0xaaaa);
...
Actually as we are going to use =Verify_Complete= in =teardown()= session in
use case of =MockIO=, we need to ensure =failWhenNotAllExpectationsUsed= not
called when already failed before. This is due to the fact that even if the test
fail during the execution, the =teardown()= is called always. So we need to make
it consistent with the current test behavior that only one test fail can be
raised in one test case.

So we wrapped the =FAIL_TEXT_C= macro with following procedure:
#+BEGIN_SRC C
...
#+END_SRC
***** Test-drive flash driver
Back to the flash driver, we can now test the given task:
#+BEGIN_SRC C
TEST(Flash, WriteSucceedsReadyImmediately)
{
  int result = 0;
...
following code make the test pass:
#+BEGIN_SRC C
int Flash_Write(ioAddress address, ioData data)
{
    IO_Write(0, 0x40);
...
#+END_SRC
Next is refactoring -- get rid all of magic numbers:
#+BEGIN_SRC C
TEST_GROUP(Flash)
{
  ioAddress address;
...
Now we proceed with the next test case in the specification list, looping case:
#+BEGIN_SRC C
...
Test all pass. So far our =Flash_Write= lives in ideal world -- error free
world. Error handling case is the next:
#+BEGIN_SRC C
...
TEST(Flash, WriteFails_BlockProtectionError)
{
  MockIO_Expect_Write(CommandRegister, ProgramCommand);
  MockIO_Expect_Write(address, data);
...
Lastly, the specification of our specific flash hardware says that until
=ReadyBit= is set, other status bits may change and should be ignored. This
specification is ensured by following test:
#+BEGIN_SRC C
TEST(Flash, WriteSucceds_IgnoresOtherBitsUntilReady)
{
  MockIO_Expect_Write(CommandRegister, ProgramCommand);
  MockIO_Expect_Write(address, data);
  MockIO_Expect_ReadThenReturn(StatusRegister, ~ReadyBit);
...
Now to move onto time out case. To do that, we need yet another test double,
fake micro timer. We want to manipulate the timer so that it can be initiated
with specified time, and incremented by specificed interval whenever we inspect
the time through the timer. After implementing the [[*Fake Micro Timer][fake micro timer]], we are good
to return to the time out case.

The specification says it can read real time clock tic to get the current
rolling \mu s counter during the ready wait loop. Let us decide the flash driver
raise timeout error if it waits more than 5 milliseconds before it got the ready
bit:
#+BEGIN_SRC C
...
Finally we check the roll backed timer:
#+BEGIN_SRC C
...
We are ready when the hardware is ready to integrate.
****** Fake Micro Timer
We'd better to specify our specification for fake
micro timer into executable spec:
#+BEGIN_SRC C
...
TEST(FakeMicroTime, SetStartTime)
{
  FakeMicroTime_Init(23, 5);
  LONGS_EQUAL(23, MicroTime_Get());
}
...
It can be thought as the nutshell of data directed programming. Build
abstraction layer around the implementation code with their (common) behavior;
then augmenting application with new feature would be accomplished by writing
some code that agree with the interface, and do the right things for the new
feature.
***** Liskov Substitution Principle
Actually, what we required from the [[*Open Closed Principle][Open Closed Principle]] is to agree with the
interface in syntactically or in type; but Liskov substitution principle also
requires the agreement even in semantics. It should provide what the API
described in their specification. So we can think of those principles usually
work together. It is easy to appreciate these concept by [[*OCP, LSP example][exercising]].
***** Interface Segregation Principle
In a nutshell, it says
#+BEGIN_QUOTE
Don't use the header file in C as header file if you are publicising those as
interface!
#+END_QUOTE

That is, use the interface as API type constraint, not as specifying all the
dependency importing code that only be used in the implementation. We should be
conscious that interface is going to be used by client.
***** Dependency Inversion Principle
It is like providing some buffer between the direct dependency. The /buffer/
should be inserted between
- high-level module depends on low-level module;
- abstraction module depends on details (implementation code).


One of example of those is OS code; it uses callback function as a form of
dependency inversion by making the callback as function pointer /interface/.

For the technique to accomplish DIP in C, we can count
- using a function pointer;
- using a interface -- header file.


To summarize, the following situations are use case of DIP:
...
- a server calls a client back through a function pointer, and
